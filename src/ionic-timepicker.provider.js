    ...
    .provider('ionicTimePicker', function () {
        ...
        this.$get = ['$rootScope', '$ionicPopup', function ($rootScope, $ionicPopup) {
            ...
            function checkAvailable() {
                var mod = 0;
                if ($scope.mainObj.format == 12 && $scope.time.meridian == 'PM') mod = 12;
                var curr = (parseInt($scope.time.hours) + mod) * 60 * 60 + $scope.time.minutes * 60;
                console.log(38, 'ionic-timepicker.provider', mod, $scope.time.meridian, $scope.time.hours);
                if(curr > $scope.mainObj.maxTime) {
                    setMinSecs($scope.mainObj.maxTime, $scope.time.format);
                } else if (curr < $scope.mainObj.minTime) {
                    setMinSecs($scope.mainObj.minTime, $scope.time.format);
                }
            }
            ...
            $scope.increaseHours = function () {
                ...
                checkAvailable();
                $scope.time.hours = ($scope.time.hours < 10) ? ('0' + parseInt($scope.time.hours)) : $scope.time.hours;
            };
